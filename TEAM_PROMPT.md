...
- 15 validated Unilog delivery fields, expandable to the full 252-column format
...
| Sourcing rule enforcement | Not present in generic pipelines | $0 | Marketplace URLs (Amazon, Grainger, eBay, Walmart) blocked at `FieldProvenanceSchema.source_url` validation layer per Unilog sourcing rules |
...
UI:           HTML5 / Vanilla JS dashboard (python3 run_ui.py → localhost:8080)
Security:     JWT + RLS + rate limiting (existing)
Cost:         ~$2 per 1,000 rows (vs $14 generic)
Output:       15 validated Unilog delivery fields, expandable to the full 252-column format
